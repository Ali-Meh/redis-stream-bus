pub use super::bus::{Stream, StreamBus};
...
impl StreamBus for RedisClient {
    ...
    async fn run<'a>(mut self, keys: &[&'a str], mut read_tx: Sender<Stream>) {
        let mut con_read = self.client.get_async_connection().await.unwrap();
        let mut con_add = self.client.get_async_connection().await.unwrap();
        let mut con_ack = self.client.get_async_connection().await.unwrap();
